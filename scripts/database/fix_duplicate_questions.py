#!/usr/bin/env python
"""
중복된 문제 이름을 가진 LeetCode 문제들의 ID를 수정하는 스크립트
k8s devops 네임스페이스 데이터베이스에 연결하여 실행
"""
...
def find_duplicate_questions(cursor):
    """중복된 이름을 가진 문제들을 찾습니다."""
    print("\n=== 중복된 문제 이름 찾기 ===")
    ...
    cursor.execute("""
        SELECT title_en, COUNT(*) as count, 
               ...
               ARRAY_AGG(csv_id ORDER BY created_at) as csv_ids
        FROM quiz_question 
        WHERE title_en IS NOT NULL AND title_en != ''
        GROUP BY title_en 
        HAVING COUNT(*) > 1
        ORDER BY count DESC, title_en
    """)
    
    duplicates = cursor.fetchall()
    print(f"중복된 영어 제목이 있는 문제 수: {len(duplicates)}")
...
def get_correct_leetcode_mapping():
    ...
    return {
        ...
        # 숫자 없이 제목만 있는 경우들 (LeetCode 문제명만)
...
def fix_duplicate_questions(cursor, conn):
    """중복된 문제들의 ID를 수정합니다."""
    print("\n=== 중복된 문제 ID 수정 시작 ===")
    
    duplicates = find_duplicate_questions(cursor)
    correct_mapping = get_correct_leetcode_mapping()
    
    updated_count = 0
    
    for duplicate in duplicates:
        title_en = duplicate['title_en']
        question_ids = duplicate['question_ids']
        csv_ids = duplicate['csv_ids']
        
        print(f"\n--- '{title_en}' ({len(question_ids)}개 중복) ---")
        
        # 올바른 LeetCode ID 찾기
        correct_id = correct_mapping.get(title_en)
        if not correct_id:
            print(f"  ⚠️  매핑에서 올바른 ID를 찾을 수 없음: {title_en}")
            continue
            
        print(f"  올바른 LeetCode ID: {correct_id}")
        
        # 각 중복 문제에 대해 수정
        for i, question_id in enumerate(question_ids):
            # csv_ids 배열이 question_ids보다 작을 수 있으므로 안전하게 처리
            current_csv_id = csv_ids[i] if i < len(csv_ids) else None
            
            # UUID 형식 검증 (36자리 하이픈 포함 문자열)
            if len(str(question_id)) != 36 or str(question_id).count('-') != 4:
                print(f"  ⚠️  ID {question_id}: 잘못된 UUID 형식, 건너뜀")
                continue
            
            if current_csv_id == correct_id:
                print(f"  ✅ ID {question_id}: 이미 올바른 ID ({current_csv_id})")
                continue
            ...
            try:
                ...
                print(f"  ✅ ID {question_id}: {current_csv_id} → {correct_id}")
                updated_count += 1
                
            except Exception as e:
                print(f"  ❌ ID {question_id}: 업데이트 실패 - {e}")
                # 트랜잭션 롤백 후 재시작
                conn.rollback()
...
def verify_changes(cursor):
    ...
    # 수정 후 중복 확인
    duplicates_after = find_duplicate_questions(cursor)
    
    if len(duplicates_after) == 0:
        print("✅ 모든 중복 문제가 해결되었습니다!")
    else:
        print(f"⚠️  여전히 {len(duplicates_after)}개의 중복이 남아있습니다.")
        for dup in duplicates_after[:5]:  # 처음 5개만 출력
            print(f"  - {dup['title_en']} ({dup['count']}개)")

def main():
    """메인 함수"""
    print("=== 중복된 LeetCode 문제 ID 수정 스크립트 ===")
    ...
    try:
        # 중복 문제 수정
        updated_count = fix_duplicate_questions(cursor, conn)
