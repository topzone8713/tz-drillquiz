...
```
ci/
└── backup/                     # Velero 백업 관련 파일
    ├── VELERO_BACKUP.md       # 이 문서
    ...
    ├── velero-config.yaml     # Velero 설정 파일
    ├── velero-jenkins-backup.sh      # Jenkins 백업 스크립트
    ├── velero-jenkins-backup.yaml     # Jenkins 백업 설정 YAML
    └── velero-jenkins-schedule.yaml   # Jenkins 자동 백업 스케줄
```
...
```bash
# 기본 백업 실행
./ci/backup/velero-backup.sh

# 특정 이름으로 백업
./ci/backup/velero-backup.sh -b my-postgres-backup

# 7일 보관 기간으로 백업
./ci/backup/velero-backup.sh -r 7

# 백업 목록 조회
./ci/backup/velero-backup.sh -l

# 백업 상세 정보 조회
./ci/backup/velero-backup.sh -d devops-postgres-backup-20231201-120000
```
...
```bash
# YAML 파일로 백업 생성
kubectl apply -f ci/backup/velero-backup.yaml
...
```bash
# 백업에서 복원
./ci/backup/velero-restore.sh devops-postgres-backup-20231201-120000

# 다른 네임스페이스로 복원
./ci/backup/velero-restore.sh -t devops-restored devops-postgres-backup-20231201

# 강제 덮어쓰기로 복원
./ci/backup/velero-restore.sh -f devops-postgres-backup-20231201

# 백업 목록 조회
./ci/backup/velero-restore.sh -l
```
...
```bash
# YAML 파일로 복원 생성
kubectl apply -f ci/backup/velero-restore.yaml
...
```bash
# 스케줄 백업 생성
kubectl apply -f ci/backup/velero-schedule.yaml
...
./ci/backup/velero-backup.sh -b prod-postgres-backup-$(date +%Y%m%d)

# 2. 백업 상태 확인
./ci/backup/velero-backup.sh -d prod-postgres-backup-20231201-120000

# 3. 운영 PostgreSQL 복원 (드라이 런)
./ci/backup/velero-restore.sh -d prod-postgres-backup-20231201-120000

# 4. 운영 PostgreSQL 복원 (실제 실행)
./ci/backup/velero-restore.sh -f prod-postgres-backup-20231201-120000
...
#### PodVolumeBackup 실패 (Kopia 리포지토리 초기화 실패)
**증상**: 
- `error to connect to repository: repository not initialized in the provided storage`
- `BackupRepository.velero.io "jenkins-default-kopia" is invalid: spec.resticIdentifier: Required value`
- `Repo maintenance error: failed to prune repo: error to connect backup repo`

**원인**: 
1. **Velero v1.17.0 버그**: BackupRepository를 자동 생성할 때 `resticIdentifier`를 올바르게 설정하지 못함
2. BackupRepository의 `resticIdentifier`가 잘못된 경로(`restic/namespace`)를 가리키고 있음
3. Kopia를 사용할 때는 `kopia/namespace` 경로를 사용해야 함

**해결 방법**:

**방법 1: BackupRepository 수동 생성 (권장)**
```bash
# 1. 기존 BackupRepository 삭제
kubectl delete backuprepositories jenkins-default-kopia -n velero
kubectl delete backuprepositories devops-default-kopia -n velero

# 2. MinIO 버킷의 잘못된 디렉토리 정리
kubectl exec -n devops minio-0 -- sh -c "MINIO_ROOT_USER=<console-user> MINIO_ROOT_PASSWORD=<console-password> mc alias set test http://localhost:9000 <s3-access-key> <s3-secret-key> && mc rm -r --force test/devops-velero/restic 2>&1 || echo 'restic directory cleaned'"

# 3. BackupRepository 수동 생성 (올바른 resticIdentifier 경로 사용)
cat <<EOF | kubectl apply -f -
apiVersion: velero.io/v1
kind: BackupRepository
metadata:
  name: jenkins-default-kopia
  namespace: velero
  labels:
    velero.io/repository-type: kopia
    velero.io/storage-location: default
    velero.io/volume-namespace: jenkins
spec:
  backupStorageLocation: default
  repositoryType: kopia
  volumeNamespace: jenkins
  maintenanceFrequency: 1h0m0s
  resticIdentifier: s3:http://minio-svc.devops.svc.cluster.local:9000/devops-velero/kopia/jenkins
---
apiVersion: velero.io/v1
kind: BackupRepository
metadata:
  name: devops-default-kopia
  namespace: velero
  labels:
    velero.io/repository-type: kopia
    velero.io/storage-location: default
    velero.io/volume-namespace: devops
spec:
  backupStorageLocation: default
  repositoryType: kopia
  volumeNamespace: devops
  maintenanceFrequency: 1h0m0s
  resticIdentifier: s3:http://minio-svc.devops.svc.cluster.local:9000/devops-velero/kopia/devops
EOF

# 4. BackupRepository 상태 확인
kubectl get backuprepositories -n velero
kubectl get backuprepositories -n velero -o jsonpath='{range .items[*]}{.metadata.name}{"\t"}{.status.phase}{"\t"}{.spec.resticIdentifier}{"\n"}{end}'

# 5. 새로운 백업 시도
./ci/backup/velero-jenkins-backup.sh
```

**방법 2: 진단 및 확인**
```bash
# 1. PodVolumeBackup 상태 확인
kubectl get podvolumebackups -n velero -l velero.io/backup-name=<backup-name>

# 2. Velero 서버 로그 확인
kubectl logs -n velero -l component=server --tail=200 | grep -i "resticIdentifier\|repository\|kopia"

# 3. BackupRepository 상태 확인
kubectl get backuprepositories -n velero -o yaml | grep -A 5 "resticIdentifier"

# 4. MinIO 인증 정보 확인 (중요!)
kubectl get secret credentials-velero -n velero -o jsonpath='{.data.cloud}' | base64 -d

# 5. Velero 서버 및 node-agent 재시작
kubectl rollout restart deployment/velero -n velero
kubectl rollout restart daemonset/node-agent -n velero
```

**주의사항**:
- **MinIO 콘솔 인증 정보 ≠ S3 API 인증 정보**
  - MinIO 콘솔 인증 정보: `MINIO_ROOT_USER` / `MINIO_ROOT_PASSWORD` (웹 콘솔 접근용)
  - S3 API 인증 정보: `aws_access_key_id` / `aws_secret_access_key` (Velero 백업용)
  - `credentials-velero` secret에는 **S3 API 인증 정보**를 사용해야 합니다
- **resticIdentifier 경로 형식**:
  - ❌ 잘못된 경로: `s3:.../restic/jenkins` (Restic용)
  - ✅ 올바른 경로: `s3:.../kopia/jenkins` (Kopia용)
- BackupRepository가 `Ready` 상태가 되어야 PodVolumeBackup이 성공합니다
- MinIO 버킷(`devops-velero`)에 대한 접근 권한이 필요합니다

**알려진 이슈**:
- **Velero v1.17.0 버그**: BackupRepository를 자동 생성할 때 `resticIdentifier`를 올바르게 설정하지 못함
- 해결 방법: BackupRepository를 수동으로 생성하고 올바른 `resticIdentifier` 경로(`kopia/namespace`)를 설정
- 이 문제는 Velero v1.17.0에서 확인되었으며, 향후 버전에서 수정될 예정입니다
...
# MinIO 버킷 확인
kubectl exec -n devops deployment/minio -- mc ls /data/devops-velero
```
