...
start_tunnel() {
    ...
    # Start SSH tunnel (must be run from Vagrantfile directory)
    (cd "$VAGRANT_DIR" && vagrant ssh kube-master -- -L $LOCAL_PORT:127.0.0.1:6443 -N -f)
...
copy_kubeconfig() {
    ...
    # Must run vagrant command from Vagrantfile directory
    (cd "$VAGRANT_DIR" && vagrant ssh kube-master -c "sudo cat $KUBECONFIG_VM_PATH") > /tmp/kubeconfig_temp 2>/dev/null
