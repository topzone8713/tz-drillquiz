...
# 기본 정보:
# 방법 1 (외부 접속): Host: db-dev.drillquiz.com, Port: 30432
...
# env 파일에서
USE_POSTGRES=true
POSTGRES_HOST=db-dev.drillquiz.com
POSTGRES_PORT=30432
...
export POSTGRES_HOST=db-dev.drillquiz.com
...
```bash
# 도메인을 사용한 접속
PGPASSWORD='DevOps!323' psql -h db-dev.drillquiz.com -p 30432 -U admin -d drillquiz

# 또는 환경 변수로 설정
export PGPASSWORD='DevOps!323'
psql -h db-dev.drillquiz.com -p 30432 -U admin -d drillquiz
```
...
```bash
# PostgreSQL 버전 확인
PGPASSWORD='DevOps!323' psql -h db-dev.drillquiz.com -p 30432 -U admin -d drillquiz -c "SELECT version();"

# 데이터베이스 정보 확인
PGPASSWORD='DevOps!323' psql -h db-dev.drillquiz.com -p 30432 -U admin -d drillquiz -c "SELECT current_database(), current_user;"

# 테이블 목록 확인
PGPASSWORD='DevOps!323' psql -h db-dev.drillquiz.com -p 30432 -U admin -d drillquiz -c "\dt"
```

### DNS 설정

도메인 `db-dev.drillquiz.com`이 노드의 외부 IP로 리다이렉트되도록 DNS 설정이 필요합니다.

```bash
# DNS 확인
nslookup db-dev.drillquiz.com
# 또는
dig db-dev.drillquiz.com +short
...
# 또는 외부 접속: db-dev.drillquiz.com:30432
if USE_POSTGRES:
    DATABASES = {
        'default': {
            ...
            'HOST': os.environ.get('POSTGRES_HOST', 'db-dev.drillquiz.com'),
...
- `POSTGRES_HOST=db-dev.drillquiz.com` (외부 접속)
...
conn = psycopg2.connect(
    host=os.getenv('POSTGRES_HOST', 'db-dev.drillquiz.com'),
